...
| 模式 | 触发条件 | 策略 |
|------|---------|------|
| **纯网络搜索**（默认） | 用户没有提供本地素材 | 5个Agent全部走网络搜索，完整流程 |
| **本地语料优先** | 用户提供了PDF/transcript/字幕/文章等 | 先分析本地素材，网络搜索变为补充 |
| **纯本地语料** | 用户明确说「只用我给的素材」或分析非公众人物 | 只分析本地素材，不做网络搜索 |

**本地语料优先模式的执行逻辑**：

1. **先读本地素材**：将用户提供的文件按5个维度分类（一本书可能同时覆盖著作+对话+决策多个维度）
...
| 素材类型 | 处理方式 | 覆盖维度 |
|---------|---------|---------|
| 书籍PDF | 直接阅读提取核心论点 | 著作(01)、决策(04) |
| 演讲/访谈transcript | 分析问答模式和即兴反应 | 对话(02)、决策(04) |
| 视频字幕SRT | 同transcript处理 | 对话(02) |
| 博客/newsletter导出 | 提取系统性观点 | 著作(01) |
| 内部文档/备忘录 | 分析决策逻辑 | 决策(04) |
...
输出要求：
- 写入 [项目目录]/references/research/01-writings.md
- 每条信息标注来源URL和可信度
...
- 视频字幕获取：
  - **Step 1 下载字幕**：`bash [项目目录]/scripts/download_subtitles.sh <YouTube_URL> [输出目录]`
    - 自动优先人工字幕 → 中文 → 英文 → 自动生成字幕
    - 输出SRT/VTT文件到指定目录
  - **Step 2 清洗为纯文本**：`python3 [项目目录]/scripts/srt_to_transcript.py <input.srt> [output.txt]`
...
- 调研摘要生成（Phase 1.5用）：`python3 [项目目录]/scripts/merge_research.py <项目目录>`
  - 自动扫描 `references/research/01-05.md`，统计来源数、一手/二手占比、关键发现
  - 输出Phase 1.5检查点的markdown表格，无需手动统计
- 质量自检（Phase 4用）：`python3 [项目目录]/scripts/quality_check.py <SKILL.md路径>`
  - 自动检查6项通过标准：心智模型数量、局限性、诚实边界、内在张力、一手来源占比、反模仿检查
  - 输出逐项PASS/FAIL和总结
...
Phase 1启动前，**主动扫描 `.claude/skills/` 目录**，检查是否有可用于信息获取的skill。如果有，在调研中优先调用：
...
**执行方式**：在spawn subagent时，把可用skill的名称和用途告知agent，让agent在调研中按需调用。
...
**关键规则**：宁可生成一个诚实标注了局限的60分框架，也不要生成一个看起来完美但实际上在编造的90分框架。
